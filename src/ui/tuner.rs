use crate::dsp::pitch::NoteReading;
use crate::ui::theme;
use eframe::egui;

const IN_TUNE_CENTS: f32 = 3.0;

pub fn tuner_widget(ui: &mut egui::Ui, reading: Option<&NoteReading>) {
    let desired = egui::vec2(ui.available_width(), 110.0);
    let (rect, _) = ui.allocate_exact_size(desired, egui::Sense::hover());
    let painter = ui.painter_at(rect);
    painter.rect_filled(rect, 6.0, egui::Color32::from_rgb(10, 10, 12));

    let center_x = rect.center().x;
    // Cent gradations: center line plus ±10/±25/±50 marks.
    for cents_mark in [-50.0f32, -25.0, -10.0, 0.0, 10.0, 25.0, 50.0] {
        let x = center_x + (cents_mark / 50.0) * (rect.width() / 2.0 - 16.0);
        let height = if cents_mark == 0.0 { 30.0 } else { 14.0 };
        painter.line_segment(
            [
                egui::pos2(x, rect.bottom() - 4.0 - height),
                egui::pos2(x, rect.bottom() - 4.0),
            ],
            egui::Stroke::new(1.0, theme::TEXT_DIM.gamma_multiply(0.6)),
        );
    }

    match reading {
        Some(r) => {
            let in_tune = r.cents.abs() < IN_TUNE_CENTS;
            let color = if in_tune { theme::GREEN } else { theme::TEXT };
            painter.text(
                egui::pos2(center_x, rect.top() + 28.0),
                egui::Align2::CENTER_CENTER,
                format!("{}{}", r.name, r.octave),
                egui::FontId::proportional(36.0),
                color,
            );
            painter.text(
                egui::pos2(center_x, rect.top() + 56.0),
                egui::Align2::CENTER_CENTER,
                format!("{:+.0} cents · {:.1} Hz", r.cents, r.frequency),
                egui::FontId::monospace(12.0),
                theme::TEXT_DIM,
            );
            let needle_x =
                center_x + (r.cents.clamp(-50.0, 50.0) / 50.0) * (rect.width() / 2.0 - 16.0);
            painter.line_segment(
                [
                    egui::pos2(needle_x, rect.bottom() - 34.0),
                    egui::pos2(needle_x, rect.bottom() - 4.0),
                ],
                egui::Stroke::new(3.0, color),
            );
        }
        None => {
            painter.text(
                rect.center(),
                egui::Align2::CENTER_CENTER,
                "—",
                egui::FontId::proportional(36.0),
                theme::TEXT_DIM.gamma_multiply(0.5),
            );
        }
    }
}
